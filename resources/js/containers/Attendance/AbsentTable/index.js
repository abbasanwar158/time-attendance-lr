...
import styles from "./AbsentTable.module.scss";
...
export default function AbsentTable({absentData}) {
      ...
      <div className={styles.mainCard}>
        ...
        <TableContainer component={Paper} className={styles.table}>
          <Table className={classes.table} aria-label="custom pagination table">
            ...
            <TableBody>
              {(rowsPerPage > 0
                ? absentData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                : absentData
              ).map((row) => (
                <TableRow key={row.id}>
                  <TableCell className={styles.nameCells}>{row.name}</TableCell>
                  <TableCell className={styles.subCells}>{row.date}</TableCell>
                  <TableCell className={styles.subCellsAbsent}>{row.absent_status == 1 ? 'Absent' : null}</TableCell>
                </TableRow>
              ))}
            </TableBody>
            <TableFooter>
