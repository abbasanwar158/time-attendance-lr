...
export default function EditRequest() {
  ...
  const updateLeave = () => {
    setOpen(true);
    var today = new Date()
    fetch(`https://devbox-attendance.herokuapp.com/api/leave/requests/reply/${replyLeave[index].id}`, {
        method: 'POST',
        headers: {
      ...
      <div className={styles.mainCard}>
        <div className={styles.gridContainer}>
          ...
          <Grid item xs={12}>
            <Grid container spacing={1} className={styles.gridSubItems} >
              <Grid item xs={12} sm={4} className={styles.fieldGrid}>
                <Button id="EditRequest_update" onClick={updateLeave} variant="contained" color="primary" className={styles.saveButton}>
                  Update
                </Button>
                <Button
                  id="EditRequest_cancel"
                  variant="contained"
                  color="default"
